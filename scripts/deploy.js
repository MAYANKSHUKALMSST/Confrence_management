...
async function deployToBackupViaJump() {
  try {
    ...
    const transferCmd = `sshpass -p "${SSH_PASSWORD}" scp -o StrictHostKeyChecking=no ${remoteTar} ${BACKUP_USER}@${BACKUP_IP}:${remoteTar}`;
    ...
    const setupCmd = `sshpass -p "${SSH_PASSWORD}" ssh -o StrictHostKeyChecking=no ${BACKUP_USER}@${BACKUP_IP} '
      mkdir -p ~/roombook && 
      cd ~/roombook && 
      tar -xf ${remoteTar} && 
